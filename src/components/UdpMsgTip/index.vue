...
import { BottomLeft, CaretBottom, CaretTop, Refresh } from "@element-plus/icons-vue"
...
watch(UDP.udp_v.udp_conn_msgs, (oldv, newv) => {
  console.log(newv)
})
onMounted(async () => {})
onActivated(async () => {})
...
const refresh = () => {
  UDP.udp_v.udp_conn_msgs = [{ msg: "UDP", updown: true }]
}
</script>
...
<template>
  <div id="terminal">
    ...
    <el-button type="danger" :icon="Refresh" circle @click="refresh" />
    <div v-for="item in UDP.udp_v.udp_conn_msgs" v-bind:key="item.msg">
